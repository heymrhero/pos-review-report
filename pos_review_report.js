/**
 * POS后台评价自动抓取与报表生成脚本 (GitHub Actions CI 版)
 * 环境变量:
 *   POS_USERNAME  - POS登录账号
 *   POS_PASSWORD  - POS登录密码
 *   WECOM_WEBHOOK - 企业微信机器人Webhook (可选)
 *   COS_BASE_URL  - COS基URL (可选，如 https://tools.mrhero.cn)
 *   REPORT_DATE   - 报表日期 (可选，默认今天)
 */
...
(async () => {
  ...
  async function fetchStoreReviews(store, authState) {
    const ctx = await browser.newContext({ storageState: authState, viewport: { width: 1440, height: 900 } });
    const page = await ctx.newPage();
    try {
  ...
  try {
    ...
    for (const store of STORES) {
      ...
      const data = await fetchStoreReviews(store, authState);
      allReviews[store.id] = data;
      console.log(data.reviews.length + '条');
    }

    const dateRanges = getDateRanges(REPORT_DATE);
    const reportHtml = generateReport(allReviews, REPORT_DATE, dateRanges);
    const outPath = path.join(__dirname, 'review_report_' + REPORT_DATE + '.html');
    fs.writeFileSync(outPath, reportHtml, 'utf-8');
    console.log('\n报表: ' + outPath);
    ...
    for (const [key, range] of Object.entries(dateRanges)) {
      ...
      for (const data of Object.values(allReviews)) {
        ...
        neutral += filtered.filter(r => r.starOverall === 3).length;
        negative += filtered.filter(r => r.starOverall <= 2).length;
      }
      console.log('[' + range.label + '] 总' + total + ' 好评' + positive + ' 中评' + neutral + ' 差评' + negative);
    }

    if (WECOM_WEBHOOK) {
      console.log('\n--- 推送企业微信 ---');
      await pushToWecom(allReviews, REPORT_DATE, dateRanges, outPath);
    }

    const ghs = process.env.GITHUB_STEP_SUMMARY;
    if (ghs) {
      let sm = '# 评价日报 ' + REPORT_DATE + '\n\n';
      for (const [key, range] of Object.entries(dateRanges)) {
        ...
        sm += '- **' + range.label + '**: 总' + total + ' 好评' + positive + ' 差评' + negative + '\n';
      }
      fs.appendFileSync(ghs, sm);
    }
...
async function pushToWecom(allReviews, baseDate, dateRanges, outputPath) {
  ...
  const payload = JSON.stringify({ msgtype: 'markdown', markdown: { content: markdown.join('\n') } });
    ...
    const req = https.request(opts, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          if (resp.errcode === 0) console.log('✓ 企业微信推送成功');
          else console.log('✗ 推送失败: ' + body);
        } catch (e) { console.log('推送响应: ' + body); }
        resolve();
      });
    });
    req.on('error', (e) => { console.log('推送网络错误: ' + e.message); resolve(); });
    req.write(payload);
    req.end();
  });
}
...
function generateReport(allReviews, baseDate, dateRanges) {
  ...
  function reviewItemHtml(review) {
    ...
    return '<div class="review-item ' + (review.starOverall <= 2 ? 'review-negative' : '') + '">' +
